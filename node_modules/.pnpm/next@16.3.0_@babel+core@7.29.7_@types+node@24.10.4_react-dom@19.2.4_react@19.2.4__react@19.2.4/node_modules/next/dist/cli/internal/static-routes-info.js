...
Object.defineProperty(exports, "staticRoutesInfoCli", {
    enumerable: true,
    get: function() {
        return staticRoutesInfoCli;
    }
});
...
/** Human-readable column titles, in the same order as CATEGORIES. */ const CATEGORY_LABELS = {
    clientJs: 'Client JS',
...
/** App Router infrastructure entries we never report as routes. */ const SKIP_APP_ENTRIES = new Set([
    '/_global-error/page'
]);
...
function discoverRoutes(distDir) {
    const middlewareManifest = readJsonFile(_path.default.join(distDir, 'server', 'middleware-manifest.json'));
    ...
    const edgeFunctions = (middlewareManifest == null ? void 0 : middlewareManifest.functions) ?? {};
    return [
        ...discoverPagesRoutes(distDir, edgeFunctions),
        ...discoverAppRoutes(distDir, edgeFunctions),
        ...discoverMiddleware((middlewareManifest == null ? void 0 : middlewareManifest.middleware) ?? {})
    ];
}
function discoverPagesRoutes(distDir, edgeFunctions) {
    const manifest = readJsonFile(_path.default.join(distDir, 'server', 'pages-manifest.json'));
    if (!manifest) return [];
    const routes = [];
...
function discoverAppRoutes(distDir, edgeFunctions) {
    const appPathsManifest = readJsonFile(_path.default.join(distDir, 'server', 'app-paths-manifest.json'));
    ...
    const appPathRoutesManifest = readJsonFile(_path.default.join(distDir, 'app-path-routes-manifest.json'));
    const routes = [];
    for (const [internalKey, entry] of Object.entries(appPathsManifest)){
        ...
        const route = (appPathRoutesManifest == null ? void 0 : appPathRoutesManifest[internalKey]) ?? internalKey;
...
function discoverMiddleware(middleware) {
 ...
 */ function collectServerEntryFiles(distDir, serverEntry, sets) {
    const entryRel = _path.default.join('server', serverEntry) // e.g. server/app/page.js
    ;
    const entryDirRel = _path.default.dirname(entryRel) // e.g. server/app
    ;
    const entryDirAbs = _path.default.join(distDir, entryDirRel);
    // The entry .js is always part of the bundle, even if no nft.json exists.
    sets.serverBundled.add(entryRel);
    const nft = readJsonFile(_path.default.join(distDir, entryRel + '.nft.json'));
    if (!(nft == null ? void 0 : nft.files)) return;
    for (const relPath of nft.files){
        ...
        const inDistDirPath = _path.default.normalize(_path.default.join(entryDirRel, relPath));
        ...
        if (isMap) {
            ...
            sets.serverMaps.add(outsideDistDir ? _path.default.resolve(entryDirAbs, relPath) : inDistDirPath);
        } else if (outsideDistDir) {
            sets.serverUnbundled.add(_path.default.resolve(entryDirAbs, relPath));
        } else if (inDistDirPath.endsWith('.js') && !inDistDirPath.endsWith('_client-reference-manifest.js')) {
 ...
 */ function parseClientReferenceManifest(filePath) {
    ...
    if (anchorIdx === -1) {
        throw Object.defineProperty(new Error(`Could not find 'globalThis.__RSC_MANIFEST[' in ${filePath}; client reference manifest format may have changed.`), "__NEXT_ERROR_CODE", {
            value: "E1234",
            enumerable: false,
            configurable: true
        });
    ...
    const skipString = (i)=>{
        ...
        if (quote !== '"' && quote !== "'") {
            throw Object.defineProperty(new Error(`Expected string literal as entry name in ${filePath} at offset ${i}, got ${JSON.stringify(content[i])}.`), "__NEXT_ERROR_CODE", {
                value: "E1238",
                enumerable: false,
                configurable: true
            });
        ...
        throw Object.defineProperty(new Error(`Unterminated entry-name string literal in ${filePath}.`), "__NEXT_ERROR_CODE", {
            value: "E1235",
            enumerable: false,
            configurable: true
        });
    ...
    const expectChar = (i, expected)=>{
        while(i < content.length && /\s/.test(content[i]))i++;
        if (content[i] !== expected) {
            throw Object.defineProperty(new Error(`Expected '${expected}' after entry name in ${filePath} at offset ${i}, got ${JSON.stringify(content[i] ?? '<eof>')}.`), "__NEXT_ERROR_CODE", {
                value: "E1233",
                enumerable: false,
                configurable: true
            });
    ...
    while(i < content.length && /\s/.test(content[i]))i++;
    if (content[i] !== '{') {
        throw Object.defineProperty(new Error(`Expected '{' after 'globalThis.__RSC_MANIFEST[...] =' in ${filePath} at offset ${i}, got ${JSON.stringify(content[i] ?? '<eof>')}.`), "__NEXT_ERROR_CODE", {
            value: "E1237",
            enumerable: false,
            configurable: true
        });
        ...
        if (ch === '"' || ch === "'") {
            ...
            if (depth === 0) {
                ...
                try {
                    ...
                    throw Object.defineProperty(new Error(`Failed to parse JSON body of ${filePath}: ${err.message}`), "__NEXT_ERROR_CODE", {
                        value: "E1236",
                        enumerable: false,
                        configurable: true
                    });
    ...
    throw Object.defineProperty(new Error(`Unterminated JSON object in ${filePath}.`), "__NEXT_ERROR_CODE", {
        value: "E1239",
        enumerable: false,
        configurable: true
    });
 ...
 */ function addClientChunk(rawPath, sets) {
 ...
 */ function collectAppClientFiles(distDir, serverEntry, sets) {
    const entryDir = _path.default.dirname(serverEntry);
    const entryBase = _path.default.basename(serverEntry, '.js');
    const baseDir = _path.default.join(distDir, 'server', entryDir);
    const crm = parseClientReferenceManifest(_path.default.join(baseDir, `${entryBase}_client-reference-manifest.js`));
    if (crm) {
        if (crm.entryJSFiles) {
    ...
    const globalBm = readJsonFile(_path.default.join(distDir, 'build-manifest.json'));
    for (const chunk of (globalBm == null ? void 0 : globalBm.rootMainFiles) ?? [])addClientChunk(chunk, sets);
}
/**
 ...
 * defensively.
 */ function collectPagesClientFiles(distDir, route, sets) {
    var _bm_pages, _bm_pages1;
    const bm = readJsonFile(_path.default.join(distDir, 'build-manifest.json'));
    if (!bm) return;
    const chunks = [
        ...((_bm_pages = bm.pages) == null ? void 0 : _bm_pages['/_app']) ?? [],
        ...((_bm_pages1 = bm.pages) == null ? void 0 : _bm_pages1[route]) ?? [],
        ...bm.polyfillFiles ?? []
    ];
 ...
 */ function deriveSourceMaps(distDir, source, target, urlCache) {
    for (const f of source){
        const fullPath = _path.default.isAbsolute(f) ? f : _path.default.join(distDir, f);
        ...
        if (mapFromUrl) {
            ...
            const mapRel = _path.default.normalize(_path.default.join(_path.default.dirname(f), mapFromUrl));
            if (!mapRel.startsWith('..') && _fs.default.existsSync(_path.default.join(distDir, mapRel))) {
                target.add(mapRel);
                continue;
            }
        }
        ...
        const adjacentFull = _path.default.isAbsolute(adjacent) ? adjacent : _path.default.join(distDir, adjacent);
        if (_fs.default.existsSync(adjacentFull)) target.add(adjacent);
    }
}
/**
 ...
 */ function readSourceMappingURL(filePath) {
    let fd;
    try {
        fd = _fs.default.openSync(filePath, 'r');
    } catch  {
        return null;
    }
    try {
        const stat = _fs.default.fstatSync(fd);
        const len = Math.min(stat.size, 4096);
        const buf = Buffer.alloc(len);
        _fs.default.readSync(fd, buf, 0, len, stat.size - len);
 ...
 */ function collectEdgeFiles(files, sets) {
    for (const f of files){
        if (f.endsWith('.js')) sets.serverBundled.add(f);
        else if (f.endsWith('.map')) sets.serverMaps.add(f);
    }
}
/** Collect all 6 file-sets for a single route. */ function collectFiles(distDir, entry, urlCache) {
 ...
 */ function buildSizeCache(distDir, allFileSets) {
    const cache = new Map();
    for (const sets of allFileSets){
        for (const cat of CATEGORIES){
            for (const f of sets[cat]){
                ...
                const fullPath = _path.default.isAbsolute(f) ? f : _path.default.join(distDir, f);
                try {
                    const stat = _fs.default.lstatSync(fullPath);
                    cache.set(f, stat.isFile() && !stat.isSymbolicLink() ? stat.size : null);
                } catch  {
                    cache.set(f, null);
                }
            }
...
/** Sum sizes for the files in `set` using the precomputed cache. */ function measureFromCache(set, cache) {
    let count = 0;
    let bytes = 0;
    for (const f of set){
...
function measureFileSets(sets, cache) {
    const result = {};
    for (const cat of CATEGORIES){
 ...
 */ function measureSharedAvg(routeIndex, allFileSets, routeEntries, cache) {
    ...
    for (const cat of CATEGORIES){
        ...
        for (const j of peers){
            ...
            const [small, big] = mySet.size <= peerSet.size ? [
                mySet,
                peerSet
            ] : [
                peerSet,
...
/** Union of all per-route file sets. Used to compute project-wide totals. */ function mergeSets(all) {
 ...
 */ function sortValue(r, key) {
    switch(key){
        ...
        default:
            key;
            throw Object.defineProperty(new Error(`unreachable sort key: ${key}`), "__NEXT_ERROR_CODE", {
                value: "E1232",
                enumerable: false,
                configurable: true
            });
 ...
 */ function sortRoutes(routes, key) {
    ...
    routes.sort((a, b)=>sortValue(b, key) - sortValue(a, key) || a.route.localeCompare(b.route));
}
/**
 ...
 */ function toDistRelative(distDir, p) {
    return _path.default.isAbsolute(p) ? _path.default.relative(distDir, p) : p;
}
/**
 ...
 */ function fileListFor(distDir, set, sizeCache) {
    const out = [];
    for (const p of set){
...
/** File counts can be fractional in averages — print 1 decimal in that case. */ function formatCount(n) {
    return Number.isInteger(n) ? `${n}` : n.toFixed(1);
}
function formatCell(stats) {
 ...
 */ function formatSharedCell(stats) {
    if (stats == null) return 'n/a';
    if (stats.count === 0 && stats.bytes === 0) return '-';
    return `${formatCount(stats.count)} files (${Math.round(stats.percentCount)}%)` + ` / ${formatBytes(stats.bytes)} (${Math.round(stats.percentBytes)}%)`;
}
/**
 ...
 */ function annotateShared(own, shared) {
...
/** Render a fixed-width markdown table — pads each cell to align columns. */ function renderMarkdownTable(headers, rows) {
...
function printMarkdown(routes, totals) {
...
function printJson(routes, totals) {
    console.log(JSON.stringify({
        routes,
        totals
    }, null, 2));
}
async function staticRoutesInfoCli(options, directory) {
    ...
    const sortKey = options.sort ? SORT_KEYS.includes(options.sort) ? options.sort : (()=>{
        console.error(`Error: invalid --sort key '${options.sort}'. Valid keys: ${SORT_KEYS.join(', ')}.`);
        process.exit(1);
    })() : 'name';
    ...
    const dir = _path.default.resolve(directory ?? process.cwd());
    ...
    if (!_fs.default.existsSync(_path.default.join(distDir, 'BUILD_ID'))) {
        console.error(`Error: No build found at ${distDir}. Run \`next build\` first.`);
        process.exit(1);
    }
    ...
    const allFileSets = routeEntries.map((entry)=>collectFiles(distDir, entry, urlCache));
    ...
    const routeInfos = routeEntries.map((entry, i)=>{
    ...
    const displayRoutes = options.limit != null && options.limit > 0 ? routeInfos.slice(0, options.limit) : routeInfos;
    if (options.json) {
...
//# sourceMappingURL=static-routes-info.js.map