...
function summarizeMemorySamples(samples) {
    ...
    return `samples=${samples.length}, min=${formatBytes(min)}, max=${formatBytes(max)}, ` + `start=${formatBytes(first)}, end=${formatBytes(last)}, ` + `Δ=${deltaSign}${formatBytes(Math.abs(delta))}, maxPressure=${maxPressure}`;
}
/**
...
async function startTurboTraceServerCli(file, port, mcpPort) {
    ...
    mcpServer.registerTool('query_spans', {
        description: 'Query spans from a turbopack trace file. Returns spans with timing, CPU usage, attribute details, and TurboMalloc live-memory samples recorded while each span was active. Set `outputType` to "json" for machine-readable output (including the raw `memorySamples` array of `[ts_offset_ticks, bytes, pressure]` triples per span — pressure is 0 = none, higher = more memory pressure) or "markdown" (default) for a human-readable summary. Use the `parent` parameter (with an ID from a previous result) to drill into children. Results are paginated to 20 spans per page.',
        inputSchema: {
    ...
    server.listen(httpPort, '127.0.0.1', ()=>{
        console.log(`Query this trace from the command line: next internal query-trace --help --port ${httpPort}`);
