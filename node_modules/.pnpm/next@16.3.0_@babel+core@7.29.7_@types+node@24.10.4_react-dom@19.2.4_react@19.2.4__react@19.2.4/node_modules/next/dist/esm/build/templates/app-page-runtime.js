...
import { isNonHtmlSecFetchDest } from '../../server/lib/is-non-html-sec-fetch-dest' with {
    'turbopack-transition': 'next-server-utility'
};
import { UNDERSCORE_NOT_FOUND_ROUTE } from '../../shared/lib/entry-constants' with {
    'turbopack-transition': 'next-server-utility'
};
...
import { shouldServeStreamingMetadata } from '../../server/lib/streaming-metadata' with {
    'turbopack-transition': 'next-server-utility'
};
...
import { getBotType } from '../../shared/lib/router/utils/is-bot' with {
    'turbopack-transition': 'next-server-utility'
};
...
export function createAppPageEntrypoint({ tree, page, pathname, require: __next_app_require__, loadChunk: __next_app_load_chunk__, interopDefault }) {
    const __next_app__ = {
     ...
     */ const isPrefetchRSCRequest = getRequestMeta(req, 'isPrefetchRSCRequest') ?? req.headers[NEXT_ROUTER_PREFETCH_HEADER] === '1' // exclude runtime prefetches, which use '2'
        ...
        if (normalizedSrcPage === UNDERSCORE_NOT_FOUND_ROUTE && (req.method === 'GET' || req.method === 'HEAD') && !isRSCRequest && isNonHtmlSecFetchDest(req.headers['sec-fetch-dest'])) {
            res.statusCode = 404;
     ...
     */ const couldSupportPPR = checkIsAppPPREnabled(nextConfig.experimental.ppr);
        ...
        const serveStreamingMetadata = !userAgent ? true : shouldServeStreamingMetadata(userAgent, nextConfig.htmlLimitedBots);
        ...
        try {
            ...
            const responseGenerator = async ({ hasResolved, previousCacheEntry: previousIncrementalCacheEntry, isRevalidating, span, forceStaticRender = false })=>{
                ...
                try {
                    ...
                    if (fallbackMode === FallbackMode.PRERENDER && (isRoutePPREnabled ? !serveStreamingMetadata : Boolean(botType))) {
