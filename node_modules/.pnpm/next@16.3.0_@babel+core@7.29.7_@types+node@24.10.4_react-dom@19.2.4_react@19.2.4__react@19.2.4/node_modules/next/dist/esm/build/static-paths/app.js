 ...
 */ async function callGenerateStaticParams(page, generateStaticParams, parentParams, rootParamKeys, implicitTags, isStaticExport) {
    ...
    const generatedParams = await workUnitAsyncStorage.run(workUnitStore, generateStaticParams, {
        params: parentParams
    });
    if (!Array.isArray(generatedParams)) {
        throw Object.defineProperty(new Error(`Invalid value returned from generateStaticParams for "${page}". Expected an array, but received type ${getValueType(generatedParams)}. See more info here: https://nextjs.org/docs/messages/generate-static-params`), "__NEXT_ERROR_CODE", {
            value: "E1450",
            enumerable: false,
            configurable: true
        });
    ...
    if (isStaticExport && generatedParams.length === 0) {
        throw Object.defineProperty(new Error(`Page "${page}" returned an empty array from "generateStaticParams()". With "output: export", at least one route must be generated. See more info here: https://nextjs.org/docs/messages/generate-static-params`), "__NEXT_ERROR_CODE", {
            value: "E1454",
            enumerable: false,
            configurable: true
        });
    ...
    for (const [index, params] of generatedParams.entries()){
        if (!isPlainObject(params)) {
            throw Object.defineProperty(new Error(`Invalid value at index ${index} returned from generateStaticParams for "${page}". Expected an object, but received type ${getValueType(params)}. See more info here: https://nextjs.org/docs/messages/generate-static-params`), "__NEXT_ERROR_CODE", {
                value: "E1451",
                enumerable: false,
                configurable: true
            });
 ...
 */ export async function generateRouteStaticParams(segments, store, isRoutePPREnabled, rootParamKeys, isStaticExport) {
 ...
 */ export async function buildAppStaticPaths({ dir, page, route, distDir, cacheComponents, authInterrupts, useCacheTimeout, staticPageGenerationTimeout, segments, isrFlushToDisk, cacheHandler, cacheLifeProfiles, requestHeaders, cacheHandlers, cacheMaxMemorySize, fetchCacheKeyPrefix, nextConfigOutput, ComponentMod, isRoutePPREnabled = false, buildId, deploymentId, rootParamKeys }) {
    ...
    const routeParams = await workAsyncStorage.run(store, generateRouteStaticParams, segments, store, isRoutePPREnabled, rootParamKeys, nextConfigOutput === 'export');
    ...
    const hadAllParamsGenerated = pathnameRouteParamSegments.length === 0 || routeParams.length > 0 && missingParamNames.length === 0;
    if (nextConfigOutput === 'export' && routeParams.length > 0 && !hadAllParamsGenerated) {
        throw Object.defineProperty(new Error(`Page "${page}" returned incomplete params from "generateStaticParams()". With "output: export", every params object must include all dynamic route parameters. Missing: ${missingParamNames.map((name)=>`"${name}"`).join(', ')}. See more info here: https://nextjs.org/docs/messages/generate-static-params`), "__NEXT_ERROR_CODE", {
            value: "E1455",
            enumerable: false,
            configurable: true
        });
