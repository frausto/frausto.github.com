...
const getReactCompilerPlugins = (maybeOptions, isServer, isDev, cwd)=>{
    ...
    try {
        ...
        const majorVersion = require(reactPkgPath).version.split('.')[0];
        if (majorVersion === '18') {
            target = majorVersion;
        }
    } catch  {
...
const getBabelLoader = (useSWCLoader, babelConfigFile, isServer, distDir, pagesDir, cwd, srcDir, dev, isClient, reactCompilerOptions, reactCompilerExclude)=>{
    if (!useSWCLoader) {
        ...
        const options = {
            ...
            hasJsxRuntime: true,
            reactCompilerPlugins: getReactCompilerPlugins(reactCompilerOptions, isServer, dev, cwd),
 ...
 */ const getReactCompilerLoader = (reactCompilerOptions, cwd, isServer, reactCompilerExclude, isDev)=>{
    const reactCompilerPlugins = getReactCompilerPlugins(reactCompilerOptions, isServer, isDev, cwd);
