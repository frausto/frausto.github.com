...
import { convertRootFlightRouterStateToRouteTree, resolveStaleAt, processRuntimePrefetchStream, writeDynamicRenderResponseIntoCache, writePrerenderResponseIntoCache } from '../segment-cache/cache';
    ...
    if (location !== null && metadataVaryPath !== null) {
        ...
        if (initialSeedData !== null && initialStaleTime !== undefined) {
            if (initialStaticStageByteLength !== undefined && initialFlightStreamForCache != null) {
                ...
                Promise.resolve(initialStaticStageByteLength).then(async (byteLength)=>{
                    ...
                    writePrerenderResponseIntoCache(now, FetchStrategy.PPR, staticStageResponse.f, undefined, staticStageResponse.h, staticStageResponse.r ?? null, staleAt, initialTree, initialRenderedSearch, true // isResponsePartial
                    );
                }).catch(()=>{
                ...
                resolveStaleAt(now, initialStaleTime).then((staleAt)=>{
                    writePrerenderResponseIntoCache(now, FetchStrategy.PPR, initialFlightData, undefined, initialHeadVaryParams, initialRootVaryParams ?? null, staleAt, initialTree, initialRenderedSearch, false // isResponsePartial
                    );
                }).catch(()=>{
                // The static stage processing failed. Not fatal — the page
                // rendered normally, we just won't write into the cache.
        ...
        if (initialRuntimePrefetchStream != null) {
            processRuntimePrefetchStream(Date.now(), initialRuntimePrefetchStream, initialTree, initialRenderedSearch).then((processed)=>{
                if (processed !== null) {
                    writeDynamicRenderResponseIntoCache(Date.now(), FetchStrategy.PPRRuntime, processed.flightDatas, processed.buildId, processed.isResponsePartial, processed.headVaryParams, processed.rootVaryParamsIterable, processed.staleAt, processed.navigationSeed, null);
                }
            }).catch(()=>{
