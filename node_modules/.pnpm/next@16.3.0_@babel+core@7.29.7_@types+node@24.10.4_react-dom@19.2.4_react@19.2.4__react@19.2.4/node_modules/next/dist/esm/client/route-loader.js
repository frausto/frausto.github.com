...
export function getClientBuildManifest() {
    ...
    return resolvePromiseWithTimeout(onBuildManifest, markAssetError(Object.defineProperty(new Error('Failed to load client build manifest'), "__NEXT_ERROR_CODE", {
        value: "E273",
        enumerable: false,
        configurable: true
    })), devBuildPromise);
...
function getFilesForRoute(assetPrefix, route) {
    if (process.env.NODE_ENV === 'development') {
        const scriptUrl = assetPrefix + '/_next/static/chunks/pages' + encodeURIPath(getAssetPathFromRoute(route, '.js')) + getAssetTokenQuery();
    ...
    return getClientBuildManifest().then((manifest)=>{
        ...
        return {
            scripts: allFiles.filter((v)=>v.endsWith('.js')).map((v)=>__unsafeCreateTrustedScriptURL(v) + getAssetTokenQuery()),
            css: allFiles.filter((v)=>v.endsWith('.css')).map((v)=>v + getAssetTokenQuery())
        };
    });
}
