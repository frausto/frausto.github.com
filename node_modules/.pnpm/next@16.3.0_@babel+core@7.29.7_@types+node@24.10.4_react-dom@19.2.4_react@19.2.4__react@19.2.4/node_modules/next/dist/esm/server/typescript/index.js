...
export const createTSPlugin = ({ typescript: ts })=>{
    function create(info) {
        ...
        proxy.getCompletionEntryDetails = (fileName, position, entryName, formatOptions, source, preferences, data)=>{
            const entryCompletionEntryDetails = entryConfig.getCompletionEntryDetails(entryName, data, fileName);
        ...
        proxy.getQuickInfoAtPosition = (fileName, position, ...args)=>{
            const prior = info.languageService.getQuickInfoAtPosition(fileName, position, ...args);
            if (!isAppEntryFile(fileName)) return prior;
            ...
            const overridden = entryConfig.getQuickInfoAtPosition(fileName, position, prior);
            ...
            ts.forEachChild(source, (node)=>{
                ...
                if (ts.isImportDeclaration(node)) {
                    ...
                    if (isAppEntry) {
                        ...
                        const metadataDiagnostics = isClientEntry ? metadata.client.getSemanticDiagnosticsForExportVariableStatement(fileName, node) : metadata.server.getSemanticDiagnosticsForExportVariableStatement(fileName, node);
                    ...
                    // export { ... }
                    if (isAppEntry) {
                        const metadataDiagnostics = isClientEntry ? metadata.client.getSemanticDiagnosticsForExportDeclaration(fileName, node) : metadata.server.getSemanticDiagnosticsForExportDeclaration(fileName, node);
                        prior.push(...metadataDiagnostics);
                    }
                    if (isServerEntry) {
