...
function findGitBoundary(cwd) {
    ...
    const gitDir = findUp.sync((dir)=>{
        var _statSync;
        const candidate = join(dir, '.git');
        const isDirectory = (_statSync = statSync(candidate, {
...
function findNextRootCandidate(cwd) {
    ...
    const marker = findUp.sync(findWorkspaceMarker, {
        cwd
    });
    if (marker) {
        return {
    ...
    const lockFile = findUp.sync(LOCK_FILES, {
        cwd
    });
    return lockFile ? {
        path: lockFile,
...
export function findRootDirAndLockFiles(cwd) {
    ...
    return {
        lockFiles: acceptedCandidates.filter((candidate)=>LOCK_FILES.includes(basename(candidate))),
        rootDir: acceptedCandidates.length ? dirname(acceptedCandidates[acceptedCandidates.length - 1]) : cwd,
        boundary
    };
}
...
export function warnRootBoundary({ boundary, marker, root, type }) {
    const config = process.env.TURBOPACK ? 'turbopack.root' : 'outputFileTracingRoot';
    ...
    Log.warnOnce(`Warning: Next.js ignored ${basename(marker)} in ${root} because ${reason}.\n` + ` To use this directory, set \`${config}\` in your Next.js config.\n`);
}
