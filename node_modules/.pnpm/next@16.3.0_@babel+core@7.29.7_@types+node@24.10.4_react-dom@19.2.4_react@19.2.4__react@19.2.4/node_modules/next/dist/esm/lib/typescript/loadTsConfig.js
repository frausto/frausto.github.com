...
function resolveConfigDirValue(value, configDir, rootConfigDir) {
    return path.resolve(configDir, value.replace(/\$\{configDir\}/g, rootConfigDir));
}
...
function resolvePackageTsConfig(extendsPath, currentConfigDir) {
    ...
    try {
        ...
        if (typeof packageJson.tsconfig === 'string') {
            return resolveConfigFile(path.resolve(path.dirname(packageJsonPath), packageJson.tsconfig));
        }
        return resolveConfigFile(path.join(path.dirname(packageJsonPath), 'tsconfig'));
    } catch  {}
}
 ...
 */ export function loadTsConfigOptions(configPath) {
    const resolvedPath = path.resolve(configPath);
    return loadTsConfigOptionsRecursive(resolvedPath, new Set(), path.dirname(resolvedPath));
}
