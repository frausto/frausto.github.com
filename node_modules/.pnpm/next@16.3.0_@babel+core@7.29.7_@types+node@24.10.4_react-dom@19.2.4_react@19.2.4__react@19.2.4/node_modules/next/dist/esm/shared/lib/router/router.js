...
import { MATCHED_PATH_HEADER, NEXT_NAV_DEPLOYMENT_ID_HEADER } from '../../../lib/constants';
...
function fetchNextData({ dataHref, inflightCache, isPrefetch, hasMiddleware, isServerRender, parseJSON, persistCache, isBackground, unstable_skipClientCache }) {
    ...
    const getData = (params)=>fetchRetry(dataHref, isServerRender ? 3 : 1, {
             ...
             */ if (!isServerRender) {
                ...
                let dplResponseHeader = response.headers.get(NEXT_NAV_DEPLOYMENT_ID_HEADER);
...
export default class Router {
   ...
   */ push(url, as, options = {}) {
        if (isJavaScriptURLString(url.toString()) || as && isJavaScriptURLString(as.toString())) {
   ...
   */ replace(url, as, options = {}) {
    ...
    async change(method, url, as, options, forcedScroll) {
        ...
        // dynamic filter.
        if (!isQueryUpdating && !options.shallow && process.env.__NEXT_CLIENT_ROUTER_FILTER_ENABLED && this._bfl_d && isDynamicRoute(pathname)) {
            const concretePathname = removeTrailingSlash(parseRelativeUrl(cleanedAs).pathname);
            const isShadowedByAppRoute = hasDynamicFilterCandidate(pathname, concretePathname, (candidate)=>!!this._bfl_d?.contains(candidate));
