...
const DEPRECATED_CUSTOM_SERVER_METHOD_GUIDANCE = {
    ...
    render: 'Please use `app.getRequestHandler()` with an adjusted parsed URL instead.',
    renderToHTML: 'Please use `app.getRequestHandler()` with an adjusted parsed URL instead.',
    renderError: 'Please use `app.getRequestHandler()` with an adjusted parsed URL instead.',
    renderErrorToHTML: 'Please use `app.getRequestHandler()` with an adjusted parsed URL instead.',
    render404: 'Please use `app.getRequestHandler()` with an adjusted parsed URL instead.'
};
function warnDeprecatedCustomServerMethod(method) {
    _log.warnOnce(`The \`app.${method}()\` method is deprecated in custom servers. ${DEPRECATED_CUSTOM_SERVER_METHOD_GUIDANCE[method]}`);
}
class NextServer {
    constructor(options){
    ...
    getRequestHandler() {
        return async (req, res, parsedUrl)=>{
            ...
            return tracer.withPropagatedContext(req.headers, ()=>tracer.trace(_constants2.NextServerSpan.getRequestHandler, async ()=>{
   ...
   */ getRequestHandlerWithMetadata(meta) {
        return async (req, res, parsedUrl)=>{
            const tracer = (0, _tracer.getTracer)();
            return tracer.withPropagatedContext(req.headers, ()=>tracer.trace(_constants2.NextServerSpan.getRequestHandlerWithMetadata, async ()=>{
