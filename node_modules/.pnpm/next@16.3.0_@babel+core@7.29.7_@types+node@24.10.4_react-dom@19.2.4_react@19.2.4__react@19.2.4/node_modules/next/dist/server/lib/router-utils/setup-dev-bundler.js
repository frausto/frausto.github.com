...
async function startWatcher(opts) {
    ...
    if (opts.nextConfig.experimental.lockDistDir) {
        ...
        const appUrl = process.env.__NEXT_PRIVATE_ORIGIN ?? `http://localhost:${opts.port}`;
        const serverInfo = {
            pid: process.pid,
            port: opts.port,
         ...
         */ if (/[\\\\/]_middleware$/.test(pageName)) {
            ...
            try {
                ...
                if (enabledTypeScript) {
                    ...
                    await (0, _rootparamstypeutils.writeRootParamsTypes)(routeTypesManifest, _path.default.join(distTypesDir, 'root-params.d.ts'));
                }
                if (!resolved) {
                    resolve();
                    resolved = true;
