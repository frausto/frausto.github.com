 ...
 */ function getTreeViewDisplayPath(pagePath) {
    const prerenderPathname = (0, _getmetadataroute.getStaticMetadataPrerenderPathname)(pagePath.startsWith('/') ? pagePath : `/${pagePath}`);
    return prerenderPathname ?? pagePath;
}
function buildStaticMetadataStaticPaths(page) {
    let pathname = (0, _apppaths.normalizeAppPath)(page);
    if (pathname.endsWith('/route')) {
        pathname = pathname.slice(0, -'/route'.length);
    }
    const prerenderPathname = (0, _getmetadataroute.getStaticMetadataPrerenderPathname)(pathname);
    if (!prerenderPathname) {
        throw Object.defineProperty(new Error(`Invariant: expected static metadata route to have a prerender pathname (${page})`), "__NEXT_ERROR_CODE", {
            value: "E1280",
            enumerable: false,
            configurable: true
        });
    ...
    return isPageStaticSpan.traceAsyncFn(async ()=>{
        ...
        if (pageType === 'app') {
            ...
            if (route.dynamicSegments.length > 0 && !pathIsEdgeRuntime) {
                ...
                if (routeModule.definition.kind === _routekind.RouteKind.APP_ROUTE && (0, _ismetadataroute.isStaticMetadataFile)(pathname)) {
                    ;
                    ({ prerenderedRoutes, fallbackMode: prerenderFallbackMode } = buildStaticMetadataStaticPaths(page));
                } else {
                    ;
