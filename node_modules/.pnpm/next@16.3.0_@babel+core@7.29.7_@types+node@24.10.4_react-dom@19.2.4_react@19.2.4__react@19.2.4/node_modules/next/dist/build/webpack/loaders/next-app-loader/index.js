...
const nextAppLoader = async function nextAppLoader() {
   ...
   */ const getStaticSiblingSegments = (segmentPath)=>{
    ...
    const header = `import { instrumentModuleGetter } from 'next/dist/server/app-render/module-loading/instrument-module-getter'\n` + collectedDeclarations.map(([varName, modulePath])=>{
        return `const ${varName} = instrumentModuleGetter(() => import(/* webpackMode: "eager" */ ${JSON.stringify(modulePath)}));\n`;
    }).join('');
    return header + code;
};
