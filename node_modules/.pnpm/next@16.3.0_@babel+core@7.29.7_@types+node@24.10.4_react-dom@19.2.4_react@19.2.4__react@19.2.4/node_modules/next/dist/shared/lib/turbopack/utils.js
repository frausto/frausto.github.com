...
function formatIssue(issue) {
    const { filePath, title, description, detail, source, importTraces, severity } = issue;
    let { documentationLink } = issue;
    let formattedTitle = renderStyledStringToErrorAnsi(title).replace(/\n/g, '\n    ');
    if (severity === 'bug' || severity === 'error' || severity === 'fatal') {
        formattedTitle = (0, _picocolors.bold)(`${(0, _picocolors.red)('Error')}: ${formattedTitle}`);
    } else if (severity === 'warning') {
        formattedTitle = (0, _picocolors.bold)(`${(0, _picocolors.yellow)('Warning')}: ${formattedTitle}`);
    } else {
        formattedTitle = (0, _picocolors.bold)(`Info: ${formattedTitle}`);
    }
