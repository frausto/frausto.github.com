...
/**
 * The social image for a page, as an explicit metadata entry.
 *
 * Next names the images generated by `opengraph-image.tsx` after the route
 * segment with no file extension, and GitHub Pages then serves them as
 * `application/octet-stream`, which stops social crawlers from rendering the
 * preview. `scripts/postbuild-og.mjs` renames them to `*.png`; pointing the
 * metadata at that name here means the built HTML and the RSC payload both ship
 * the final URL, so nothing has to be rewritten after the fact.
 */
export function ogImage(pathname = '/') {
